		...
		function showCloud($returnType = 'html') {
			...
			if (is_array($this->wordsArray)) {
				...
				foreach ($this->wordsArray as $word => $arrayInfo) {	
					//$yomomma = $this->getClassFromPercent((($arrayInfo['size'] - $this->min)/ ($this->max - $this->min)) * 100) . " " . $arrayInfo['size'] . " " . $this->min . " " . " " . $this->max;
					if(($this->max - $this->min) != 0){
						$sizeRange = $this->getClassFromPercent((($arrayInfo['size'] - $this->min)/ ($this->max - $this->min)) * 100);
					}
					else{
						$sizeRange = 0;
					}
					//echo "<script language='javascript'> alert('$yomomma') </script>";
					...
					if ($returnType == 'array') {
                        ...
                        $blue = 255 - $red;
                        $green = 125;
						$return .= "<span class='word size{$sizeRange}'> &nbsp; <a style='color:rgb($red,$green,$blue); text-decoration: none;' href=\"results.php?query={$arrayInfo['word']}\"'>{$arrayInfo['word']}</a> &nbsp; </span>";
					}
				}
				return $return;
			}
