...
var jumpMap={},count=0,prev,curr;$(".browse-list__category").each(function(){curr=$(this).attr("data-sort-letter");count+=1;curr!=prev&&(jumpMap[curr]=count);prev=curr});$(".jumplist__item").each(function(){$(this).attr("data-sort-letter")in jumpMap||$(this).css("display","none")}).on("click",function(){paging.setPage(Math.ceil(jumpMap[$(this).attr("data-sort-letter")]/$(".browse-list__category").length));$(".jumplist").find(".active").removeClass("active");$(this).addClass("active");return!1});$(".jumplist").removeClass("hidden");
var listId=$("#browse-results"),categorizedResults=$("#browse-results > li");pagingOpts.onSelect=function(a){updatePage(listId,categorizedResults,this.slice);showPager(this.pages)};var paging=$(".pagination").paging(totalHeadings,pagingOpts);paging.setOptions({onSelect:function(a){updatePage(listId,categorizedResults,this.slice);showPager(this.pages)}});function updatePage(a,b,c){b=b.slice(c[0],c[1]);collapseAll($("#collapse-expand-all"));a.empty();a.append(b)};
