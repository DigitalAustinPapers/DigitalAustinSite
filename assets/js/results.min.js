var basicData=null,cloudData=null,cityData=null,sortKey="date",positiveThreshold=2,negativeThreshold=-2;function queryChanged(){cityData=cloudData=basicData=null;requestData();jQuery("#tabs").removeClass("hidden");return!1}
...
c+=" sent to: <b>"+placeIdToNames[b]+"</b>");document.getElementById("sentiment").value&&(c+=" with <b>"+document.getElementById("sentiment").value+"</b> sentiment scores");document.getElementById("humanQuery").innerHTML=c;$.getJSON("data/search.php"+a,function(a){$(document).trigger("basicDataLoaded",[a])});$.getJSON("data/cloud.php"+a,function(a){$(document).trigger("cloudDataLoaded",[a])});$.getJSON("data/cities.php"+a,function(a){$(document).trigger("cityDataLoaded",[a])});$.getJSON("data/network.php"+
a,function(a){$(document).trigger("networkDataLoaded",[a])});return!0}function updateDocuments(){var a=$("#documentsList");a.empty();$("#resultsCount").text(basicData.length);$("#totalDocsCount").text(totalDocsCount);$(a).load("search_results.php",{basicData:basicData});$("#sort_"+sortKey).prop("checked",!0)}$(document).on("basicDataLoaded",function(a,c){null!=c&&c!=basicData?(basicData=c,updateDocuments()):alert("?")});
$("input:radio[name=sort]").on("click",function(a){sortKey=$("input:radio[name=sort]:checked").val();queryChanged()});var mapIsSetup=!1,map,markers=[],lines=[],infoWindows=[];google.load("maps","3",{other_params:"sensor=false"});
...
function redrawAllCurves(){for(var a=0;a<lines.length;a++)lines[a].setMap(null);lines=[];for(a=0;a<basicData.length;a++)if(doc=basicData[a],null!=doc.srcLat&&null!=doc.dstLat){var c=.1+.2*Math.random();drawCurve(map,new google.maps.LatLng(doc.srcLat,doc.srcLng),new google.maps.LatLng(doc.dstLat,doc.dstLng),c)}}
...
function updateChart(){if(null!=basicData){for(var a={},c={},b={},g={},f=0;f<basicData.length;f++){var d=basicData[f].date.substring(0,4);void 0===a[d]?a[d]=1:a[d]++;void 0===c[d]&&(c[d]=0,b[d]=0,g[d]=0);basicData[f].sentimentScore<negativeThreshold?g[d]++:basicData[f].sentimentScore>positiveThreshold?c[d]++:b[d]++}var f=2E3,a=1E3,e;for(e in totalDocDistribution)15<=parseInt(totalDocDistribution[e])&&(d=parseInt(e),d<f&&(f=d),parseInt(e)>a&&(a=d));for(var h=[["Year","Negative","Neutral","Positive"]];f<=
a;f++)e=f.toString(),d=0,void 0!==totalDocDistribution[e]&&(d=parseInt(totalDocDistribution[e])),h.push([e,Math.floor(g[e]/d*1E3)/10,Math.floor(b[e]/d*1E3)/10,Math.floor(c[e]/d*1E3)/10])}c=google.visualization.arrayToDataTable(h);timeChart.draw(c,{isStacked:!0,title:"Search Results, Distributed by Year",hAxis:{title:"Year"},vAxis:{title:"Percentage out of all documents",minValue:0},series:[{color:"#b70000"},{color:"gray"},{color:"#006600"}]});google.visualization.events.addListener(timeChart,"select",
function(){console.log(timeChart.getSelection()[0]);var a=timeChart.getSelection()[0];if("row"in a&&"column"in a){var b=h[a.row+1][0];1==a.column&&(document.getElementById("sentiment").value="negative");2==a.column&&(document.getElementById("sentiment").value="neutral");3==a.column&&(document.getElementById("sentiment").value="positive");document.getElementById("fromYear").value=b;document.getElementById("toYear").value=b;queryChanged();$("#tabs").tabs("option","active",0)}})}
