...
a+="&toPlaceId=";a+=encodeURIComponent(document.getElementById("toPlaceId").value);a+="&sentiment=";a+=encodeURIComponent(document.getElementById("sentiment").value);a+="&sort=";a+=encodeURIComponent(sortKey);ga("send","event","search","submit",a);var b="";document.getElementById("query").value&&(b="text: <b>"+document.getElementById("query").value+"</b>");document.getElementById("fromYear").value&&(b+=" starting: <b>"+document.getElementById("fromYear").value+"</b>");document.getElementById("toYear").value&&
(b+=" ending: <b>"+document.getElementById("toYear").value+"</b>");if(document.getElementById("fromPersonId").value)var c=document.getElementById("fromPersonId").value,b=b+(" sender: <b>"+personIdToNames[c]+"</b>");document.getElementById("toPersonId").value&&(c=document.getElementById("toPersonId").value,b+=" recipient: <b>"+personIdToNames[c]+"</b>");document.getElementById("fromPlaceId").value&&(c=document.getElementById("fromPlaceId").value,b+=" sent from: <b>"+placeIdToNames[c]+"</b>");document.getElementById("toPlaceId").value&&
(c=document.getElementById("toPlaceId").value,b+=" sent to: <b>"+placeIdToNames[c]+"</b>");document.getElementById("sentiment").value&&(b+=" with <b>"+document.getElementById("sentiment").value+"</b> sentiment scores");""==b&&(b="all results.");document.getElementById("humanQuery").innerHTML=b;$.getJSON("search_results.php"+a,function(a){$(document).trigger("basicDataLoaded",[a])});$.getJSON("data/cloud.php"+a,function(a){$(document).trigger("chartDataLoaded",[a])});$.getJSON("data/cities.php"+a,
function(a){$(document).trigger("cityDataLoaded",[a])});$.getJSON("data/network.php"+a,function(a){$(document).trigger("networkDataLoaded",[a])});return!0}
function updateDocuments(){$("#documentsList");var a=basicData.json.length;$("#resultsCount").text(a);$("#totalDocsCount").text(totalDocsCount);$(".search-results__results-summary").removeClass("invisible");1===a&&(document.getElementById("resultsPlural").innerHTML="result");$(".search-results__results-summary").addClass("search-results__results-summary--changed").delay(2E3).queue(function(){$(this).removeClass("search-results__results-summary--changed").dequeue()});$(".pagination").paging(a,pagingOpts);
...
function updateSentiment(){$(".search-result-list__item-sentiment-score").each(function(){var a=$(this),b=a.attr("data-sentiment");b<negativeThreshold?a.addClass("sentiment-negative"):b>positiveThreshold?a.addClass("sentiment-positive"):a.addClass("sentiment-neutral")})}
var pagingOpts={format:"[< nncnn >]",perpage:20,lapping:0,page:1,onSelect:function(a){updatePage(this.slice);updateSentiment();a=$(".pagination");1<this.pages?a.removeClass("hidden"):a.hasClass("hidden")||a.addClass("hidden")},onFormat:function(a){switch(a){case "block":return this.page===this.value?'<li class="active"><a href="#">'+this.value+"</a></li>":'<li><a href="#">'+this.value+"</a></li>";case "next":return'<li><a href="#" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
case "prev":return'<li><a href="#" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';case "first":return'<li><a href="#">first</a></li>';case "last":return'<li><a href="#">last</a></li>'}}};function updatePage(a){var b=$("#documentsList"),c=basicData.html.slice(a[0],a[1]);b.empty().append(c).first().attr("start",a[0]+1)}var mapIsSetup=!1,map,markers=[],lines=[],infoWindows=[];google.load("maps","3",{other_params:"sensor=false"});
