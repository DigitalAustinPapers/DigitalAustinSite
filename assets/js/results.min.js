...
function updateClouds(){$("#wordCloud").empty();$("#personCloud").empty();$("#placeCloud").empty();wordChart(cloudData[0],"#wordCloud");$("#personCloud").jQCloud(cloudData[2]);$("#placeCloud").jQCloud(cloudData[1]);cloudsNeedRerender=!1}$(document).on("cloudDataLoaded",function(a,c){null!=c&&c!=cloudData&&(cloudData=c,cloudsNeedRerender=!0,"none"!=$("#tab-clouds").css("display")&&updateClouds())});
function wordChart(a,c){function b(){var a=d3.scale.linear().domain([0,d3.max(g,function(a){return parseInt(a.weight,10)})]).range([0,d]);d3.scale.linear().range([f+5,0]);f=25*g.length-e.top-e.bottom;var b=d3.select(c).append("svg").attr("width",d+e.left+e.right).attr("height",f+e.top+e.bottom).append("g").attr("transform","translate("+e.left+","+e.top+")").selectAll("g").data(g).enter().append("g").attr("transform",function(a,b){return"translate("+e.left+","+25*b+")"});b.append("rect").attr("fill",
"red").attr("class","bar").attr("width",function(b){return a(b.weight)}).attr("height",19);b.append("text").attr("x",function(b){return a(b.weight-3)}).attr("y",10).attr("fill","red").attr("dy",".35em").text(function(a){return a.weight});b.append("text").attr("class","label").attr("x",function(a){return-10}).attr("y",10).attr("dy",".35em").text(function(a){return a.text}).attr("color","black")}var g=a,e={top:30,right:10,bottom:30,left:10},d=parseInt(d3.select(c).style("width"),10),d=d-e.left-e.right,
f=200;b.width=function(a){if(!arguments.length)return d;d=a;return b};b.height=function(a){if(!arguments.length)return f;f=a;return b};return b()}var timeChart,timeChartNeedsUpdate=!0;google.load("visualization","1",{packages:["corechart"],callback:function(){timeChart=new google.visualization.ColumnChart(document.getElementById("timeChart"))}});
