var basicData=null,chartData=null,cityData=null,sortKey="date",positiveThreshold=2,negativeThreshold=-2,resultsListItems=null;function queryChanged(){resultsListItems=cityData=chartData=basicData=null;requestData();jQuery(".result-tabs").removeClass("hidden");return!1}
...
c+=" sent to: <b>"+placeIdToNames[b]+"</b>");document.getElementById("sentiment").value&&(c+=" with <b>"+document.getElementById("sentiment").value+"</b> sentiment scores");document.getElementById("humanQuery").innerHTML=c;$.getJSON("search_results.php"+a,function(a){$(document).trigger("basicDataLoaded",[a])});$.getJSON("data/cloud.php"+a,function(a){$(document).trigger("chartDataLoaded",[a])});$.getJSON("data/cities.php"+a,function(a){$(document).trigger("cityDataLoaded",[a])});$.getJSON("data/network.php"+
...
function drawCurve(a,c,b,e){var g=a.getProjection();c=g.fromLatLngToPoint(c);b=g.fromLatLngToPoint(b);var d=b.x-c.x,f=b.y-c.y,h=Math.sqrt(Math.pow(d,2)+Math.pow(f,2)),h=h*e;e=Math.atan2(f,d)+Math.PI/2;d=new google.maps.Point((c.x+b.x)/2,(c.y+b.y)/2);e=new google.maps.Point(d.x+h*Math.cos(e),d.y+h*Math.sin(e));for(var l,k=!0,h=0;10>=h;h+=1){var d=h/10,f=Math.pow(1-d,2)*c.x+2*(1-d)*d*e.x+Math.pow(d,2)*b.x,m=Math.pow(1-d,2)*c.y+2*(1-d)*d*e.y+Math.pow(d,2)*b.y,f=g.fromPointToLatLng(new google.maps.Point(f,
...
function redrawMarkers(){console.log("RedrawMarkers called.");if(null==cityData)console.log("Aborting."),console.log(cityData);else{for(var a=0;a<markers.length;a++)markers[a].setMap(null);markers=[];infoWindows=[];var c;c=7<map.getZoom()?cityData.length:Math.min(cityData.length,Math.ceil(Math.pow(map.getZoom(),1.2)));for(a=0;a<c;a++){var b=cityData[a],e=new google.maps.Marker({position:new google.maps.LatLng(b.lat,b.lng),map:map,title:b.name}),g=function(){cityClicked(arguments.callee.markerIndex)};
g.markerIndex=a;google.maps.event.addListener(e,"click",g);markers.push(e);b="<a style='color:#0000FF;text-decoration:underline;' onClick='searchCity("+b.id+")'> "+b.name+"</a><br /><b>"+(parseInt(b.incoming)+parseInt(b.outgoing))+" Letters</b><br />"+b.incoming+" Incoming Letters<br />"+b.outgoing+" Outgoing Letters<br />";b=new google.maps.InfoWindow({content:b});infoWindows.push(b)}mapNeedsRerender=!1}}var chartsNeedRerender=!1;
function updateCharts(){$("svg").remove();wordChart(chartData[2],"#personChart");wordChart(chartData[1],"#placeChart");chartsNeedRerender=!1}$(document).on("chartDataLoaded",function(a,c){null!=c&&c!=chartData&&(chartData=c,chartsNeedRerender=!0,"none"!=$("#tab-charts").css("display")&&updateCharts())});
function wordChart(a,c){var b={top:30,right:10,bottom:30,left:10},e=parseInt(d3.select(c).style("width"),10),e=e-b.left-b.right,g=25*a.length-b.top-b.bottom;console.log(d3.select(c).style("width"));console.log(b);console.log(e);var d=d3.scale.linear().domain([0,d3.max(a,function(a){return parseInt(a.weight,10)})]).range([0,e-125-40]);d3.scale.linear().domain([0,a.length]).range([g+5,0]);var e=d3.select(c).append("svg").attr("width",e+b.left+b.right).attr("height",g+b.top+b.bottom).append("g").attr("transform",
"translate("+[b.left,b.top]+")"),f=e.selectAll("g").data(a).enter().append("g").attr("transform",function(a,c){return"translate("+(b.left+125)+","+25*c+")"});d3.select(e.node().parentNode).style("height",g+b.top+b.bottom+"px");f.append("rect").attr("fill","red").attr("class","bar").attr("width",function(a){return d(a.weight)}).attr("height",19);f.append("text").attr("x",function(a){return d(a.weight-3)}).attr("y",10).attr("fill","black").attr("dy",".35em").text(function(a){return a.weight});f.append("text").attr("class",
"label").attr("x",function(a){return-125}).attr("y",10).attr("dy",".35em").text(function(a){return a.text}).attr("color","black")}var timeChart,timeChartNeedsUpdate=!0;google.load("visualization","1",{packages:["corechart"],callback:function(){timeChart=new google.visualization.ColumnChart(document.getElementById("timeChart"))}});
function updateChart(){if(null!=basicData){for(var a={},c={},b={},e={},g=0;g<basicData.json.length;g++){var d=basicData.json[g].date.substring(0,4);void 0===a[d]?a[d]=1:a[d]++;void 0===c[d]&&(c[d]=0,b[d]=0,e[d]=0);basicData.json[g].sentimentScore<negativeThreshold?e[d]++:basicData.json[g].sentimentScore>positiveThreshold?c[d]++:b[d]++}var g=2E3,a=1E3,f;for(f in totalDocDistribution)15<=parseInt(totalDocDistribution[f])&&(d=parseInt(f),d<g&&(g=d),parseInt(f)>a&&(a=d));for(var h=[["Year","Negative",
"Neutral","Positive"]];g<=a;g++)f=g.toString(),d=0,void 0!==totalDocDistribution[f]&&(d=parseInt(totalDocDistribution[f])),h.push([f,Math.floor(e[f]/d*1E3)/10,Math.floor(b[f]/d*1E3)/10,Math.floor(c[f]/d*1E3)/10])}c=google.visualization.arrayToDataTable(h);timeChart.draw(c,{isStacked:!0,title:"Search Results, Distributed by Year",hAxis:{title:"Year"},vAxis:{title:"Percentage out of all documents",minValue:0},series:[{color:"#b70000"},{color:"gray"},{color:"#006600"}]});google.visualization.events.addListener(timeChart,
