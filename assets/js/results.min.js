...
function wordChart(a,c){var b=parseInt(d3.select(c).style("width"),10),b=b-10-10,g=25*a.length-30-30,e=d3.scale.linear().domain([0,d3.max(a,function(a){return parseInt(a.weight,10)})]).range([0,b-125-40]);d3.scale.ordinal().domain(a.map(function(a){return a.text})).rangeBands([g,0],.1);var b=d3.select(c).append("svg").attr("class","word-chart__outer-svg").attr("width",b+10+10).attr("height",g+30+30).append("g").attr("class","word-chart__inner-g").attr("transform","translate(10,30)"),d=b.selectAll("g").data(a).enter().append("g").attr("transform",
function(a,b){return"translate(135,"+25*b+")"});d3.select(b.node().parentNode).style("height",g+30+30+"px");d.append("rect").attr("class","word-chart__bar").attr("width",function(a){return e(a.weight)}).attr("height",19);d.append("text").attr("class","word-chart__item-weight").attr("x",function(a){return e(a.weight-3)}).attr("y",10).attr("dy",".35em").text(function(a){return a.weight});b.append("g").attr("class","word-chart__label-container").selectAll("text").data(a).enter().append("a").attr("class",
"word-chart__label-link").attr("xlink:href",function(a){return a.link}).append("text").attr("class","word-chart__label-text").attr("transform",function(a,b){return"translate(0,"+25*b+")"}).attr("dy",".75em").text(function(a){return a.text})}var timeChart,timeChartNeedsUpdate=!0;google.load("visualization","1",{packages:["corechart"],callback:function(){timeChart=new google.visualization.ColumnChart(document.getElementById("timeChart"))}});
