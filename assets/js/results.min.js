...
function updateSentiment(){$(".search-result-list__item-sentiment-score").each(function(){var a=$(this),b=a.attr("data-sentiment");b<negativeThreshold?a.addClass("sentiment-negative"):b>positiveThreshold?a.addClass("sentiment-positive"):a.addClass("sentiment-neutral")})}pagingOpts.perpage=20;pagingOpts.onSelect=function(a){updatePage(this.slice);updateSentiment();showPager(this.pages)};
function updatePage(a){var b=$("#documentsList"),c=basicData.html.slice(a[0],a[1]);b.empty().append(c).first().attr("start",a[0]+1)}var mapIsSetup=!1,map,markers=[],lines=[],infoWindows=[];google.load("maps","3",{other_params:"sensor=false"});
