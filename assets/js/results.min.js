...
function updateClouds(){$("svg").remove();wordChart(cloudData[2],"#personCloud");wordChart(cloudData[1],"#placeCloud");cloudsNeedRerender=!1}$(document).on("cloudDataLoaded",function(a,c){null!=c&&c!=cloudData&&(cloudData=c,cloudsNeedRerender=!0,"none"!=$("#tab-clouds").css("display")&&updateClouds())});
function wordChart(a,c){var b={top:30,right:10,bottom:30,left:10},e=parseInt(d3.select(c).style("width")),e=e-b.left-b.right,g=25*a.length-b.top-b.bottom;console.log(d3.select(c).style("width"));console.log(b);console.log(e);var d=d3.scale.linear().domain([0,d3.max(a,function(a){return parseInt(a.weight,10)})]).range([0,e]);d3.scale.linear().domain([0,a.length]).range([g+5,0]);var e=d3.select(c).append("svg").attr("width",e+b.left+b.right+"px").attr("height",g+b.top+b.bottom+"px").append("g").attr("transform",
"translate("+[b.left,b.top]+")"),f=e.selectAll("g").data(a).enter().append("g").attr("transform",function(a,c){return"translate("+(b.left+50)+","+25*c+")"});d3.select(e.node().parentNode).style("height",g+b.top+b.bottom+"px");f.append("rect").attr("fill","red").attr("class","bar").attr("width",function(a){return d(a.weight)}).attr("height",19);f.append("text").attr("x",function(a){return d(a.weight-3)}).attr("y",10).attr("fill","red").attr("dy",".35em").text(function(a){return a.weight});f.append("text").attr("class",
"label").attr("x",function(a){return-50}).attr("y",10).attr("dy",".35em").text(function(a){return a.text}).attr("color","black")}var timeChart,timeChartNeedsUpdate=!0;google.load("visualization","1",{packages:["corechart"],callback:function(){timeChart=new google.visualization.ColumnChart(document.getElementById("timeChart"))}});
